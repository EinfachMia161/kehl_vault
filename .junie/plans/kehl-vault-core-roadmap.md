...
### ✓ Step 11: Multi-vault profile and database switcher
Enable isolated vaults and profiles for different contexts (e.g., Personal, Work, Finance).

- Implement `profile.h` / `profile.c` to list, switch, create, and manage independent encrypted vault files.
- Add CLI interface options to switch active profile seamlessly without restarting the application.
- Ensure all comments and documentation are strictly in English.

### ✓ Step 12: Entry categorization and tag management
Add structured taxonomy to organize credentials effectively.

- Implement `category.h` / `category.c` supporting default categories (Logins, Cards, Secure Notes, Identity) and custom user-defined tags.
- Provide category-based filtering and listing views in the CLI.

### ✓ Step 13: Time-Based One-Time Password (TOTP / 2FA) authenticator
Implement RFC 6238 TOTP generator enabling two-factor authentication code generation.

- Implement Base32 decoding, HMAC-SHA1 / HMAC-SHA256 calculation, and 30-second time-step rolling code calculation in `totp.h` / `totp.c`.
- Integrate TOTP view and real-time remaining seconds counter in the CLI.

### ✓ Step 14: Entry version history, change tracking, and trash bin
Prevent accidental loss by maintaining historical revisions and soft deletion.

- Implement `history.h` / `history.c` to store timestamped previous revisions when updating entries.
- Add Trash Bin functionality for restoring soft-deleted entries or permanently purging them.

### ✓ Step 15: Password expiration, age tracking, and security alerts
Track credential freshness and alert users when passwords become stale or reach their expiration policy.

- Implement `expiry.h` / `expiry.c` tracking creation/modification timestamps and configured expiry intervals (e.g. 30, 60, 90 days).
- Add expiration dashboard in the security audit view.

### ✓ Step 16: Encrypted snapshot manager and automatic backup rotation
Provide automated snapshot creation and rotation to prevent catastrophic data loss.

- Implement `backup.h` / `backup.c` to create timestamped encrypted copies on save and rotate older backups.
- Provide snapshot restore options in the CLI.

### ✓ Step 17: Fuzzy search and multi-field query engine
Enhance credential retrieval with fast fuzzy string matching and multi-field search.

- Implement `search.h` / `search.c` with Levenshtein / substring scoring across title, username, category, and tags.
- Integrate interactive query search in the CLI.

### ✓ Step 18: In-memory secure memory zeroization and wiping
Harden RAM security against memory dumps and cold boot attacks.

- Implement `secure_mem.h` / `secure_mem.c` using `SecureZeroMemory` on Windows and `explicit_bzero` / volatile memory scrubbing on POSIX.
- Guarantee sensitive string buffers (passwords, encryption keys, auth tags) are zeroized immediately upon scope exit.

### ✓ Step 19: Comprehensive English codebase commenting and documentation refactor
Ensure every single source and header file is thoroughly commented in English for maximum clarity and maintainability.

- Add clear, instructional English header comments and function-level documentation across all `.c`, `.h`, and `.cpp` files.
- Update `CODEBASE_DOCUMENTATION.md` in English detailing the complete 20-step architecture.

### ✓ Step 20: Automated verification suite expansion, build, and Git sync
Validate the entire expanded architecture through comprehensive unit tests and automated regression checks.

- Expand `tests/test_vault.cpp` covering all new modules (profiles, categories, TOTP, history, expiry, backup, search, secure mem).
- Execute the full test suite and verify clean builds across all targets.
- Push the verified changes to the remote repository.